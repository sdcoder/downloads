...
{
    public class AffiliateProgramPageModel : AboutPagesModel
    {
        private LightStreamPageDefault defaults;
        public AffiliateProgramPageModel() : this(new ContentManager(), AppSettings.Load().PageDefault.About)
        { }

        public AffiliateProgramPageModel(ContentManager content, LightStreamPageDefault defaults) : base(content, defaults)
        {
        ...
        {
            var cmsContent = ContentManager.Get<AffiliateProgram>();
            if (cmsContent != null)
            {
                Title = cmsContent.PageTitle;
                SubHeading = cmsContent.SubHeading;
                AffiliateProgramCopy = cmsContent.AffiliateProgramCopy;
