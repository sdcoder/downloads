 ...
 * Customized for Lightstream.com - do not replace with updates without review of changes
 */
(function ($) {
...
function SignaturePad (selector, options) {
   ...
   */
  function drawIt() {
    settings.usingScriptFont = false; // LightStream change
   ...
   */
  function typeIt() {
    settings.usingScriptFont = true; // LightStream change
    ...
    // Lightstream - useless and bizzare IE8 fix.
    if ($('html').hasClass('lt-ie9')) {
        $(context).addClass('x').removeClass('x'); 
    }
  }
  ...
  function validateForm () {
    ...
    if (!settings.usingScriptFont && output.length < 1) { // LightStream - changed validation logic - allow "usingScriptFont"
      ...
      if (!settings.displayOnly) {
        ...
        if (settings.validateFields) {
          if ($(selector).is('form')) {
            ...
            // Lightstream - removed form binding
  ...
  $.extend(self, {
    ...
    , validateForm: function () { return validateForm() }
      ...
      // LightStream added
      , usingScriptFont: function () { return settings.usingScriptFont; }
  })
}
