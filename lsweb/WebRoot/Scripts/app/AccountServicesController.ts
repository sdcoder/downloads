...
(function () {
    ...
    angular.module('AccountServicesModule', ['ls.services', 'ls.filters', 'LightStreamDirectives', 'ngRoute'])
        ...
        .controller('AccountServicesController', ['$scope', '$http', '$sce', '$window', '$compile', function ($scope, $http, $sce, $window, $compile) {
            let modal = new LightStreamWeb.Frontend.PrintableAjaxModalController("#PrintableModal", "#PrintButton");

            $scope.Model = {};
            $scope.Loading = true;

            // private functions
            var updateApplicationById = function (applicationId, appData, isActive = false) {
                var i;
                ...
                refreshAccount = function (account, successCallback?: (result: any) => void) {
            ...
            $scope.dataToBlob = function (data: string) {
            ...
            $scope.confirmCancelExtraPayment = function () {
                // stash the active account with the modal
                var $modal = $('#CancelExtraPaymentModal'),
                    account = $modal.data('account');
