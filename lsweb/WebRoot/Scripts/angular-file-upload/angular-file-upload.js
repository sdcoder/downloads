	...
	return function(scope, elem, attr) {
		if ('draggable' in document.createElement('span')) {
			...
			elem[0].addEventListener("drop", function(evt) {
			    ...
			    elem.removeClass(elem[0].__drag_over_class_);
			    elem[0].__drag_over_class_ = null;
			    if (!extractFiles(evt, function (files) {
					fn(scope, {
			            $files : files,
			            $event : evt
			        });
			    })) {
			        var directoryDroppedFn = $parse(attr['ngFileDropIsDirectory']);
			        if (directoryDroppedFn instanceof Function) {
			            directoryDroppedFn(scope, {
			                $event: evt
			            });
			        } 
                }
			...
			function extractFiles(evt, callback) {
				...
				if (items && items.length > 0 && items[0].webkitGetAsEntry && $location.protocol() != 'file' && 
					...
					var file, fileList = evt.dataTransfer.files;
					if (fileList != null) {
					    for (var i = 0; i < fileList.length; i++) {
					        file = fileList.item(i);
					        if (!file.type && file.size % 4096 == 0) {
					            // files without a type and a multiple of 4096 bytes are almost certainly folders.
					        } else {
					            files.push(file);
					        }
						}
